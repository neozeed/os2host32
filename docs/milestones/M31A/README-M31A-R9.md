...
## R9 diagnostic boundary

R8 proved that resize/repaint is stable and that the first key reaches
WMCHAR's DrawMessage.  The remaining crash is at the indexed virtual-key table
lookup at object-1 offset 0179.  R9 verifies the actual loader relocation for
that operand and reports EAX/fault address on any access violation.
...
Look for:
    ...
    M31A FIXUP      : ... old=000202EC new=010202EC
    M31A FIXUP done : ... mem=010202EC

If the key still faults, the exception line now includes EAX and `fault=`.
